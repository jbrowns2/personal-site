...
async function getInvitationReport(sql, filters) {
    ...
    var proximityMap = buildProximityMap(data.events, codesById);

    var allInvitations = buildInvitationReport(data.codes, data.events, {});
    var invitations = buildInvitationReport(data.codes, data.events, filters || {});
    var summary = buildSummary(data.codes, data.events, allInvitations);
    var activity = buildActivity(data.events, codesById, proximityMap);
    var followUp = buildFollowUp(allInvitations, data.events, codesById, proximityMap);

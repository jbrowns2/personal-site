...
function normalizeIpForCompare(ip) {
    ...
    var m = /^::ffff:(\d{1,3}(?:\.\d{1,3}){3})$/i.exec(s);
    if (m) {
        return m[1];
    }
    return s;
...
async function issueChallenge(sql, ip) {
  ...
  `;
    ...
    RETURNING id
  `;
    return { id: String(rows[0].id), prefix: prefix, difficulty: difficulty };
}
...
async function validateChallenge(sql, challengeId, nonce, ip) {
  ...
  `;
    ...
    var rowIp = normalizeIpForCompare(row.ip);
    var reqIp = normalizeIpForCompare(ip);
    if (rowIp !== reqIp) {

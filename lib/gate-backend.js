...
const POW_BASE_DIFFICULTY = 18;
const POW_ESCALATED_DIFFICULTY = 20;
const POW_ESCALATION_THRESHOLD = 3;
...
async function issueChallenge(sql, ip) {
    ...
    var recentFails = await sql`
    SELECT COUNT(*)::int AS c FROM portfolio_gate_attempts
    WHERE ip = ${ip}::inet AND success = false
      AND attempted_at > now() - ${failWindowLabel}::interval
  `;
    var failCount = recentFails[0] ? recentFails[0].c : 0;
    var difficulty =
        failCount >= POW_ESCALATION_THRESHOLD ? POW_ESCALATED_DIFFICULTY : POW_BASE_DIFFICULTY;
    var ttlLabel = String(CHALLENGE_TTL_MINUTES) + ' minutes';
    var rows = await sql`
    INSERT INTO portfolio_gate_challenges (prefix, difficulty, ip, expires_at)
    VALUES (${prefix}, ${difficulty}, ${ip}::inet, now() + ${ttlLabel}::interval)
    RETURNING id
  `;
    return { id: rows[0].id, prefix: prefix, difficulty: difficulty };
}

async function validateChallenge(sql, challengeId, nonce, ip) {
    if (
        !challengeId ||
  ...
  `;
    ...
    var rowIp = typeof row.ip === 'string' ? row.ip.replace(/\/\d+$/, '') : String(row.ip);
    if (rowIp !== ip) {
        return { valid: false, reason: 'challenge_ip_mismatch' };
    }
    var hash = crypto
        .createHash('sha256')
    ...
    await sql`UPDATE portfolio_gate_challenges SET used = true WHERE id = ${challengeId}::uuid`;
    return { valid: true };
}

...
const VALID_EMPLOYMENT_TYPES = [EMPLOYMENT_TYPE_FULL_TIME, EMPLOYMENT_TYPE_CONTRACT];
...
function computeCodeLookupHash(normalized) {
...
function parseUserAgent(ua) {
    ...
    if (/mobile|iphone|android.*mobile|windows phone/.test(lower)) {
        deviceType = 'mobile';
    ...
    var browser = 'Unknown';
    if (lower.indexOf('edg/') >= 0 || lower.indexOf('edge/') >= 0) browser = 'Edge';
    else if (lower.indexOf('chrome/') >= 0 && lower.indexOf('chromium') < 0) browser = 'Chrome';
    else if (lower.indexOf('firefox/') >= 0) browser = 'Firefox';
    else if (lower.indexOf('safari/') >= 0 && lower.indexOf('chrome') < 0) browser = 'Safari';

    var os = 'Unknown';
    if (lower.indexOf('windows') >= 0) os = 'Windows';
    else if (lower.indexOf('mac os') >= 0 || lower.indexOf('macintosh') >= 0) os = 'macOS';
    else if (lower.indexOf('iphone') >= 0 || lower.indexOf('ipad') >= 0) os = 'iOS';
    else if (lower.indexOf('android') >= 0) os = 'Android';
    else if (lower.indexOf('linux') >= 0) os = 'Linux';

    return { deviceType: deviceType, browser: browser, os: os };
}

async function findAccessCodeByHash(sql, hash) {
    if (!sql || !hash) return null;
    var rows = await sql`
        SELECT id, label, active, expires_at, created_at, code_lookup_hash,
...
async function findAccessCodeByLookupHash(sql, lookupHash) {
    if (!sql || !lookupHash) return null;
    var rows = await sql`
        SELECT id, label, active, expires_at, created_at, code_lookup_hash
        FROM portfolio_gate_access_codes
        WHERE code_lookup_hash = ${lookupHash}
        LIMIT 1
    `;
    return rows[0] || null;
}

async function resolveFailureOutcome(sql, normalized) {
    var lookupHash = computeCodeLookupHash(normalized);
    var row = await findAccessCodeByLookupHash(sql, lookupHash);
    if (!row) {
        return { outcome: 'incorrect', codeId: null };
    }
    if (!row.active) {
        return { outcome: 'disabled_code', codeId: row.id };
    }
    if (row.expires_at && new Date(row.expires_at).getTime() < Date.now()) {
        return { outcome: 'expired_code', codeId: row.id };
    }
    return { outcome: 'incorrect', codeId: null };
}

async function recordAccessCodeEvent(sql, opts) {
    if (!sql || !opts || !opts.outcome) return;
    var ua =
        opts.userAgent && typeof opts.userAgent === 'string'
            ? opts.userAgent.slice(0, REQUEST_USER_AGENT_MAX_LEN)
            : null;
    try {
        await sql`
            INSERT INTO portfolio_gate_access_code_events
                (code_id, outcome, attempt_lookup_hash, ip, fingerprint, user_agent)
            VALUES
                (${opts.codeId || null},
                 ${opts.outcome},
                 ${opts.attemptLookupHash || null},
                 ${opts.ip || null}::inet,
                 ${opts.fingerprint || null},
                 ${ua})
        `;
        if (opts.outcome === 'success' && opts.codeId) {
            await sql`
        ...
        console.error('recordAccessCodeEvent', err && err.message);
        throw err;
    }
}

/** @deprecated Use recordAccessCodeEvent */
async function recordAccessCodeUsed(sql, hash) {
    if (!sql || !hash) return;
    try {
        var row = await findAccessCodeByHash(sql, hash);
        if (row) {
            await recordAccessCodeEvent(sql, {
                outcome: 'success',
                codeId: row.id,
                ip: null,
                fingerprint: null,
                userAgent: null,
                attemptLookupHash: null,
            });
        }
    } catch (err) {
...
function employmentTypeToTokenCode(employmentType) {
    return normalizeEmploymentType(employmentType) === EMPLOYMENT_TYPE_CONTRACT ? 'ct' : 'ft';
}

function employmentTypeFromTokenCode(code) {
    return code === 'ct' ? EMPLOYMENT_TYPE_CONTRACT : EMPLOYMENT_TYPE_FULL_TIME;
}
...
function signSession(secret, employmentType, profileSlug, codeId) {
    const exp = Math.floor(Date.now() / 1000) + SESSION_MAX_AGE_SEC;
    const nonce = crypto.randomBytes(16).toString('hex');
    const et = employmentTypeToTokenCode(employmentType);
    const ps = normalizeProfileSlug(profileSlug) || '-';
    const cid =
        codeId != null && String(codeId).trim() !== '' && Number(codeId) > 0
            ? String(Math.floor(Number(codeId)))
            : '-';
    const payload = exp + '.' + nonce + '.' + et + '.' + ps + '.' + cid;
...
function parseSessionToken(token, secret) {
    try {
        ...
        if (parts.length === 3) {
            ...
            employmentType = employmentTypeFromTokenCode(parts[2]);
        } else if (parts.length === 5) {
            employmentType = employmentTypeFromTokenCode(parts[2]);
            profileSlug = parts[3] === '-' ? null : normalizeProfileSlug(parts[3]);
        } else {
            employmentType = employmentTypeFromTokenCode(parts[2]);
            profileSlug = parts[3] === '-' ? null : normalizeProfileSlug(parts[3]);
            if (parts[4] !== '-') {
                const parsedId = Number(parts[4]);
        ...
        return { employmentType: employmentType, profileSlug: profileSlug, codeId: codeId };
    } catch (e) {
        return null;
    }
}

async function findAccessCodeById(sql, id) {
    if (!sql || id == null) {
        return null;
    }
    var rows = await sql`
        SELECT id, label, active, expires_at, employment_type, profile_slug
        FROM portfolio_gate_access_codes
        WHERE id = ${id}
        LIMIT 1
    `;
...
async function resolveSessionFromDb(sql, session) {
    if (!session || !session.codeId || !sql) {
        return session;
    }
    try {
        var row = await findAccessCodeById(sql, session.codeId);
        if (!row || !row.active) {
            return session;
        }
        if (row.expires_at && new Date(row.expires_at).getTime() < Date.now()) {
        ...
        return {
            ...
            profileSlug: normalizeProfileSlug(row.profile_slug),
...
function loadSiteProfile(slug) {
    const normalized = normalizeProfileSlug(slug);
    if (!normalized) {
        return null;
    }
    const profilesDir = path.join(__dirname, '..', 'site-profiles');
    const filePath = path.resolve(profilesDir, normalized + '.json');
    if (!filePath.startsWith(path.resolve(profilesDir) + path.sep)) {
        return null;
    }
    try {
...
function getProfileContactEmail(slug) {
    const profile = loadSiteProfile(slug);
    if (!profile || !profile.contact || typeof profile.contact.email !== 'string') {
        return null;
    }
...
function getSessionContext(req) {
    try {
        ...
        return parseSessionToken(token, secrets.secret);
    } catch (e) {
        return null;
    }
}
...
async function checkStatusRateLimit(sql, ip) {
    ...
    // Count before inserting so rate-limited clients don't keep writing rows
    // and extend their own penalty window on every retry.
    const rows = await sql`
...
async function insertAutoIssuedAccessCode(sql, opts) {
    ...
    const lookupHash = computeCodeLookupHash(opts.normalizedCode);
    const expiresIso =
    ...
    const rows = await sql`
        ...
        VALUES (${opts.label}, ${hash}, true, ${expiresIso}, ${lookupHash}, ${opts.normalizedCode})

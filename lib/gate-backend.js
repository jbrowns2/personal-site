...
function loadGateSecrets() {
    ...
    // Support multiple newline-separated bcrypt hashes so different access
    // codes can be issued per company / context (any one match unlocks).
    const bcryptHashes = raw.split('\n').map(function (s) { return s.trim(); }).filter(Boolean);
    if (bcryptHashes.length === 0) {
        return null;
    }
    return { bcryptHashes, secret };
}

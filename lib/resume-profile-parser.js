    ...
    blocks.forEach(function (block) {
        ...
        lines.forEach(function (line) {
            ...
            if (isItalicContext(t)) {
                const italicText = stripItalic(t);
                if (hasDateRange(italicText) && !dates) {
                    dates = normalizeDates(italicText.replace(/^[^·|]*[·|]\s*/, ''));
                } else {
                    context = italicText;
                }
                return;
            }
            if (!organization && !dates && t.includes('|') && !t.startsWith('-')) {
                organization = stripMd(t.split('|')[0]);
                return;
            }
            if (!organization && !t.startsWith('-') && !t.startsWith('*') && t.length < 140) {
                organization = stripMd(t);
                return;
            }
            if (!context && !t.startsWith('-') && !t.startsWith('*') && t.length >= 40) {
                context = stripMd(t);
                return;
            }
            if (t.startsWith('-')) {
                bullets.push(stripMd(t.replace(/^\s*-\s+/, '')));
            }
        });
        ...
        if (title && (bullets.length > 0 || organization)) {
            items.push({
                title: title,
                organization: organization || 'Contract assignment',
                context: context,
                dates: dates,
...
function parseFullTimeExperienceItems(body) {
    ...
    lines.forEach(function (line) {
        ...
        if (t.startsWith('-')) {
            const bullet = stripMd(t.replace(/^\s*-\s+/, ''));
...
function parseExperienceItems(text, employmentType) {
    ...
    if (employmentType === 'contract') {
        return parseContractExperienceItems(body);
    }
    return parseFullTimeExperienceItems(body);
}
...
function buildHighlights(competencies, summary) {
    const highlights = [];
    for (let i = 0; i < Math.min(4, competencies.length); i++) {
        const item = competencies[i];
    ...
    while (highlights.length < 4 && summary) {
        const fallbacks = [
            { value: '15+ Years', label: 'Regulated industries and enterprise-scale analytics delivery' },
            { value: 'MS Economics', label: 'Applied econometrics certificate and FLMI credential' },
            { value: 'Production ML', label: 'Forecasting, classification, and executive reporting platforms' },
            { value: 'Executive KPIs', label: 'Power BI, Tableau, and SSRS dashboards leadership uses daily' },
        ];
...
function buildAboutCards(experienceItems, competencies) {
    if (experienceItems.length >= 2) {
        return experienceItems.slice(0, 3).map(function (item) {
            return {
                title: item.title,
                body: item.bullets[0] || item.context || item.organization,
            };
        });
    }
    return competencies.slice(0, 3).map(function (c) {
        const words = c.split(/\s+/).slice(0, 4).join(' ');
        return { title: words, body: c };
    });
}

function buildContact(employmentType, email, role, employer, competencies) {
    if (employmentType === 'contract') {
        return {
            label: '06 / Hire',
            ...
            topics: competencies.slice(0, 4),
            engagementPills: ['Contract', 'Remote', 'Defined Deliverables'],
            valueItems: [
                {
                    label: 'Fast ramp',
                    body: 'Deep domain experience in regulated financial services',
                },
                {
...
function parseResumeToProfile(markdown, opts) {
    ...
    const experienceItems = parseExperienceItems(text, employmentType);
    ...
    const profile = {
        ...
        highlights: buildHighlights(competencies, summary),
        about: {
            title: employmentType === 'contract' ? 'Contract Profile' : 'About Jonathan Brownstein',
            lead: summary.split(/(?<=[.!?])\s+/)[0] || summary,
            body: summary,
            cards: buildAboutCards(experienceItems, competencies),
        },
        experience: {
            title: employmentType === 'contract' ? 'Delivery Record' : 'Professional Experience',
            intro:
                employmentType === 'contract'
                    ? 'Selected assignments aligned with this role.'
                    : 'Experience tailored for the ' + roleShort + ' opportunity.',
            items: experienceItems,
        },
        skills: {
            title: 'Skills & Expertise',
            intro: 'Capabilities highlighted for this application.',
        ...
        contact: buildContact(employmentType, email, role, employer, competencies),
    };

    if (profile.contact.topics && profile.contact.topics.length === 0) {
        profile.contact.topics = competencies.slice(0, 4);
    }
